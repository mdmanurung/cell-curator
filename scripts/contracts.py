...
def read_config(path: Path) -> dict[str, Any]:
    ...
    if int(value.get("schema_version", -1)) != 2:
        raise ContractError("unsupported configuration schema_version")
    assays = value.get("input", {}).get("assays")
    if not isinstance(assays, dict) or not assays:
        raise ContractError("input.assays must be a nonempty mapping")
    lanes = value.get("evidence", {}).get("lanes")
    if not isinstance(lanes, list) or not lanes:
        raise ContractError("evidence.lanes must be a nonempty list")
...
def representation_decl(
    ...
    if representation not in representations:
        raise ContractError(
            f"assay {assay_id!r} lacks representation {representation!r}"
        )
    declaration = representations[representation]
    source = str(declaration.get("source", "")).lower()
    if source not in {"x", "layer"}:
        raise ContractError(
            f"{assay_id}/{representation} source must be X or layer"
        )
    ...
    if detection is not None:
        ...
        if detection_source == "layer" and not str(detection.get("key", "")).strip():
            raise ContractError(
                f"{assay_id}/{representation} detection layer key is blank"
            )
    return declaration
        ...
        lanes.append(
            {
                ...
                "required_for_acceptance": as_bool(
                    raw.get("required_for_acceptance", True)
                ),
                "ontology_compatible": as_bool(
