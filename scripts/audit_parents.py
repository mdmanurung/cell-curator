...
def build_audit(config: dict, cells: pd.DataFrame) -> pd.DataFrame:
    ...
    for (scope, parent), frame in cells.groupby(
        ...
        row: dict[str, object] = {
            ...
            "max_capture_fraction": float(captures.iloc[0]) if not captures.empty else float("nan"),
        ...
        for guide in guides:
            ...
            high_entropy |= bool(
                np.isfinite(entropy)
                and entropy >= float(thresholds["guide_entropy_min"])
            )
            meaningful_minority |= bool(
                np.isfinite(second_fraction)
                and second_fraction
                >= float(thresholds["meaningful_minority_fraction"])
            )
    ...
    result["qc_outlier"] = (
        result[robust_metrics].abs().max(axis=1).ge(
            float(thresholds.get("technical_metric_robust_z_max", 3.0))
        )
        if robust_metrics
        else False
    )
