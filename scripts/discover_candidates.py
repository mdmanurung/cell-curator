...
def collapse_stored_families(config: dict, children: list[Child]) -> list[list[Child]]:
    ...
    for indices in by_scope_parent.values():
        ...
        for left_resolution, right_resolution in zip(
            ...
            right = [index for index in indices if children[index].resolution == right_resolution]
            for a in left:
...
def _candidate_rows_from_stored(
    ...
    for family_number, family in enumerate(
        sorted(families, key=lambda group: (group[0].scope, natural_key(group[0].parent_id), group[0].resolution)),
        1,
    ):
...
def _local_candidates(
    ...
    for (scope, parent_id), frame in local.groupby(
        ...
        for resolution in sorted(frame["resolution"].astype(float).unique()):
            ...
            valid = stable and resolving and not technical and clusters >= 2 and eligible_clusters >= 2
            messages.append(
            ...
            registry.append(
                {
                    ...
                    "n_captures": selected[capture_key].astype(str).nunique() if capture_key else 0,
                    "parent_purity": 1.0,
...
def run_discovery(
    *,
    ...
    required_parent_columns.update(
        str(keys[name])
        for name in ("donor", "capture")
        if str(keys.get(name, "")).strip()
    )
    require_columns(
        parents,
    ...
    candidate_parents = {
        (str(row["scope"]), str(row["parent_id"])) for row in registry
    }
    triage_rows = []
    ...
    manifest = {
        ...
        "invariants": {
            "every_flagged_parent_routed": not triage[
                triage["audit_flagged"]
            ]["triage_action"].eq("").any(),
