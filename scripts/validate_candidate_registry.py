...
def validate_registry(
    ...
    for row in registry.itertuples(index=False):
        ...
        if eligibility.get("min_donors") is not None and int(row.n_donors) < int(eligibility["min_donors"]):
            raise ContractError("candidate donor count is below the configured minimum")
        if eligibility.get("min_captures") is not None and int(row.n_captures) < int(eligibility["min_captures"]):
        ...
        block = membership[
            membership["candidate_key"].astype(str).eq(str(row.candidate_key))
        ]
    ...
    routed = set(zip(registry["scope"].astype(str), registry["parent_id"].astype(str)))
    flagged = {
...
def main() -> None:
    ...
    try:
        ...
        atomic_write_json(
            args.output, validate_registry(config, audit, registry, membership)
        )
