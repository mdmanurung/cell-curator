...
def build_binary_groups(
    *,
    ...
    if comparator_role == "parent_remainder":
        ...
        comparator = set(
            scope_cells.loc[scope_cells["parent_id"].astype(str).eq(sibling), "barcode"].astype(
                str
            )
        )
    if not comparator or target & comparator:
        raise ContractError("binary comparator is empty or overlaps the candidate")
    selected = scope_cells[scope_cells["barcode"].astype(str).isin(target | comparator)].copy()
    selected["analysis_group"] = (
        selected["barcode"].astype(str).map(lambda barcode: "0" if barcode in target else "1")
    )
    ...
    for _, candidate in registry.iterrows():
        for comparator_role in COMPARATOR_ROLES:
            ...
            target_hash = values_sha256(
                sorted(groups.loc[groups["analysis_group"].eq("0"), "barcode"].astype(str))
            )
            comparator_hash = values_sha256(
                sorted(groups.loc[groups["analysis_group"].eq("1"), "barcode"].astype(str))
            )
            for lane in lanes:
                assay = str(lane["assay"])
                rep = str(lane["representation"])
                lane_id = f"{candidate['candidate_key']}__{comparator_role}__{lane['lane_key']}"
                rows.append(
                    {
                        "lane_id": lane_id,
                        "scope": str(candidate["scope"]),
                        ...
                        "required_for_acceptance": bool(lane["required_for_acceptance"]),
