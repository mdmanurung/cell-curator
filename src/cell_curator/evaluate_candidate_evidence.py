...
def evaluate_candidates(config: dict, evidence: pd.DataFrame) -> pd.DataFrame:
    ...
    conditional = {
        ...
        "incompatible_coexpression_fraction": audit.get("incompatible_program_fraction_min"),
    }
    ...
    for row in evidence.to_dict("records"):
        ...
        gates["minimum_cells"] = int(row["n_cells"]) >= int(eligibility["min_cells"])
        optional_checks = (
            ...
            (
                "capture_nondominance",
                "max_capture_fraction",
                audit.get("capture_max_fraction"),
                "<",
            ),
            (
                "technical_confounding",
                "technical_effect_size",
                audit.get("technical_effect_size_max"),
                "<",
            ),
            ...
            (
                ...
                audit.get("incompatible_program_fraction_min"),
                "<",
            ),
        )
        for gate_name, column, threshold, operator in optional_checks:
            ...
            gates[gate_name] = (
                value >= float(threshold) if operator == ">=" else value < float(threshold)
            )
...
def decide_parents(audit: pd.DataFrame, children: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    for parent in audit.to_dict("records"):
        ...
        if not accepted.empty:
            ...
            audit_thresholds.get("doublet_fraction_max") is not None
            and float(parent["doublet_fraction"])
            >= float(audit_thresholds["doublet_fraction_max"])
        ) or (
            audit_thresholds.get("incompatible_program_fraction_min") is not None
            and float(parent["incompatible_program_fraction"])
            >= float(audit_thresholds["incompatible_program_fraction_min"])
        ):
        ...
        rows.append(
            {
                ...
                "confidence": "high"
                if outcome in {"ACCEPT SPLIT", "DOUBLET/MIXED"}
                else "moderate",
...
def main() -> None:
    ...
    try:
        ...
        evidence = pd.read_csv(
            args.candidate_evidence, sep="\t", dtype=str, keep_default_na=False
        )
