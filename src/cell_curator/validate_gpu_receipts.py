...
def validate_receipt(config: dict, lane: pd.Series, receipt: dict, root: Path) -> None:
    ...
    if str(runtime.get("rapids_singlecell")) != str(marker["rapids_singlecell_version"]):
        raise ContractError("RAPIDS version drifted")
    if str(runtime.get("rapids_distribution")) != str(marker["rapids_distribution"]):
...
def validate_all(config: dict, lanes: pd.DataFrame, receipts_root: Path) -> dict:
    ...
    for candidate, block in lanes.groupby("candidate_key", observed=True):
        ...
        if observed != expected or len(block) != len(expected):
            raise ContractError(f"candidate {candidate} lacks configured binary evidence lanes")
    ...
    for _, lane in rapids.iterrows():
        ...
        validate_receipt(config, lane, json.loads(receipt_path.read_text()), receipts_root)
    return {
        "schema_version": 3,
...
def main() -> None:
    ...
    try:
        ...
        lanes = pd.read_csv(args.lane_registry, sep="\t", dtype=str, keep_default_na=False)
