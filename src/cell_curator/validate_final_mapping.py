...
def construct_mapping(
    *,
    ...
    require_columns(
        canonical, {"barcode", "scope", "parent_id"}, artifact="canonical membership"
    )
    require_columns(
        decisions,
    ...
    canonical_parents = set(
        zip(
            canonical["scope"].astype(str),
            canonical["parent_id"].astype(str),
            strict=True,
        )
    )
    decision_parents = set(
        zip(
            decisions["scope"].astype(str),
            decisions["parent_id"].astype(str),
            strict=True,
        )
    )
...
def validate_mapping(canonical: pd.DataFrame, mapping: pd.DataFrame) -> dict:
    ...
    expected_parent_uid = (
        observed["scope"].astype(str) + "::" + observed["parent_id"].astype(str)
    )
    ...
    lineage_totals = mapping.groupby("scope", observed=True)["leaf_id"].nunique().sort_index()
    return {
        "schema_version": 3,
...
def main() -> None:
    ...
    try:
        ...
        membership = pd.read_csv(
            args.candidate_membership, sep="\t", dtype=str, keep_default_na=False
        )
        ...
        if args.build_mapping:
            ...
            if not left.equals(right):
                raise ContractError(
                    "provided mapping differs from accepted candidate decisions"
                )
