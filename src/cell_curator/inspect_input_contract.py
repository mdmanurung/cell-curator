...
def _inspect_obs(adata, config: dict, *, is_mudata: bool) -> dict:
    ...
    return {
        ...
        "sorted_barcode_sha256": values_sha256(list(adata.obs_names), sort_values=True),
        "is_mudata": is_mudata,
    }
...
def _inspect_assay(config: dict, assay_id: str, adata) -> tuple[dict, list[str]]:
    ...
    for graph in assay.get("graph_representations", []):
        ...
        if source == "obsm":
            if key not in adata.obsm:
                raise ContractError(f"assay {assay_id!r} graph representation is absent: {key}")
            shape = adata.obsm[key].shape
        elif source in {"x", "layer"}:
            shape = _matrix(adata, graph).shape
        else:
            raise ContractError(f"assay {assay_id!r} graph source must be X, layer, or obsm")
    ...
    for name in assay["representations"]:
        ...
        detection_summary: dict[str, Any] = {"available": False, "required": False}
        ...
        representations[str(name)] = {
            ...
            "ontology_compatible": bool(declaration.get("ontology_compatible", False)),
    ...
    if kind == "h5ad":
        ...
        try:
            ...
            for assay_id, assay in assays.items():
                ...
                assay_summaries[assay_id], found = _inspect_assay(config, assay_id, adata)
...
def validate_mode(config: dict) -> tuple[str, list[str]]:
    ...
    locally_grounded = any(
        str(item.get("kind", "")) in {"local_markers", "reference", "cached_table"}
        for item in providers
    )
    require_local = bool(
        config.get("knowledge", {}).get("require_local_grounding_for_labels", True)
    )
    if mode == "annotation" and require_local and not locally_grounded:
        raise ContractError("annotation mode requires a configured local knowledge provider")
    blockers = []
    if not locally_grounded:
        blockers.append(
            "No local knowledge provider is configured: labels and write-back remain blocked; evidence-only analysis is available"
        )
...
def run(config_path: Path, output_root: Path) -> dict:
    ...
    expected = sum(
        int(scope["expected_parent_count"]) for scope in config["canonical_partitions"].values()
    )
