...
def _labels_present(frame: pd.DataFrame) -> bool:
    columns = [
        ...
        if str(column).startswith("biological_label_") or column in {"cell_type", "cl_id"}
    ]
    return any(
        frame[column].fillna("").astype(str).str.strip().ne("").any() for column in columns
    )
